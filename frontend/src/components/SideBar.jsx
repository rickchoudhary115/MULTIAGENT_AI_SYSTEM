  ...
  if (collapsed) {
    return (
      <div className="hidden lg:flex flex-col items-center w-[56px] h-screen bg-[#0d0f14] border-r border-white/[0.06] py-4 gap-1 shrink-0">
        <button
          className="flex items-center justify-center  w-9 h-9 rounded-lg text-slate-500 hover:text-slate-200 hover:bg-white/[0.05] transition-colors duration-150 bg-transparent border-none cursor-pointer mb-1"
        ...
        <div className="flex-1 overflow-y-auto px-2.5 pb-2 [scroller-width:none][&::-webkit-scrollbar]:hidden pt-4">
          {conversations.map((conv) => {
            ...
            return (
              ...
              >
                <div
                  className={`flex  items-center justify-center shrink-0 w-[20px] h-[20px] rounded-lg 
  ...
  return (
    <div className="fixed lg:static inset-y-0 left-0 z-50 w-[270px] h-screen shrink-0 bg-[#0d0f14] border-r border-white/[0.06]">
      <div className="flex flex-col py-3 h-full">
        ...
        <div className="flex-1 overflow-y-auto px-2.5 pb-2 [scroller-width:none][&::-webkit-scrollbar]:hidden">
          {conversations.map((conv) => {
            const isActive = selectedConversation?._id == conv?._id;
            return (
              <div
              ...
              >
                <div
                  className={`flex  items-center justify-center shrink-0 w-[20px] h-[28px] rounded-lg 
                ${isActive ? "bg-indigo-500/15 text-indigo-400" : "bg-white/[0.05] text-slate-500"}`}
                >
                  <MessageSquare size={13} />
                </div>
        ...
        <div className="mx-2.5 h-[80px] bg-white/[0.06]">
          <div className="px-3.5 py-2.5">
            {user ? (
              <div className="flex items-center justify-center gap-2.5 cursor-pointer rounded-xl px-1 py-1 hover:bg-white/[0.05] transition-colors duration-150">
                <div className="relative shrink-0">
                  {user?.avatar && !imageError ? (
                    <img
                      className="w-9 h-9 rounded-[10px] object-cover border-2 border-indigo-500/25"
              ...
              <button className="w-full flex  items-center justify-center gap-2 tetx-sm font-medium text-slate-200 bg-white/[0.05] border border-white/[0.08] rounded-xl py-[11px] cursor-pointer hover:bg-white/[0.08]  transition-colors duration-150">
                Login
              </button>
            )}
          </div>
